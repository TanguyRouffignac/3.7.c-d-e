...
class MyWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        try:
            self.scraper.browser.open('http://www.echecs.asso.fr')
        except (mechanize.HTTPError):
            label = Gtk.Label("Site de la FFE injoignable")
            self.add(label)
            self.show_all()
            Gtk.main()
            return
